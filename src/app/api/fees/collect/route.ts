/**
 * Next.js API Route - Proxy to Express x402 Server Fees Endpoint
 * 
 * This route proxies fee collection requests to the Express x402 server.
 * The Express server handles payment verification via PayAI middleware.
 * 
 * Client calls: /api/fees/collect (same origin, no CORS)
 * Next.js proxies to: Express server /api/fees/collect (server-to-server, no CORS needed)
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expressServerUrl) {
      console.error('[Fees Proxy] Express server URL not configured');
    ...
    if (!isDevelopment && expressServerUrl.includes('localhost')) {
      console.error('[Fees Proxy] Invalid Express server URL for production:', expressServerUrl);
      return NextResponse.json(
        { 
          error: 'Express server URL not configured',
    ...
    if (xPaymentHeader) {
      ...
      console.log('[Fees Proxy] X-Payment header present, forwarding to Express server');
    } else {
      console.log('[Fees Proxy] No X-Payment header - first request (will get 402)');
    }
    
    const targetUrl = `${expressServerUrl}/api/fees/collect`;
    
    console.log('[Fees Proxy] Forwarding fee collection request to Express server:', targetUrl);
    ...
    try {
      ...
      console.log('[Fees Proxy] Express server response:', {
      ...
      console.error('[Fees Proxy] Fetch error:', {
    ...
    if (response.status === 502) {
      ...
      console.error('[Fees Proxy] Railway 502 error from Express server:', errorText);
      return NextResponse.json(
        { 
    ...
    console.error('[Fees Proxy] Unexpected error:', error);
    return NextResponse.json(
      { 
        error: 'Proxy error',
