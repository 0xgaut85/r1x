 ...
 * 
 * This route proxies plan requests to the Express x402 server.
 * The Express server handles payment verification via PayAI middleware.
 * 
 * Client calls: /api/r1x-agent/plan (same origin, no CORS)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expressServerUrl) {
      console.error('[Next.js Proxy] Express server URL not configured');
      return NextResponse.json(
        { 
          error: 'Express server URL not configured',
          message: 'X402_SERVER_URL environment variable is missing.',
        },
        { status: 500 }
      );
    }
    ...
    const targetUrl = `${expressServerUrl}/api/r1x-agent/plan`;
    
    console.log('[Next.js Proxy] Forwarding plan request to Express server:', targetUrl);
