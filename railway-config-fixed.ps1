...
try {
    ...
    if ($expressService) {
        ...
        # Update service settings (root directory)
        ...
        # Update Next.js service with X402_SERVER_URL if we have the URL
        if ($expressUrl -and $nextjsService) {
        ...
        } elseif (-not $expressUrl) {
            Write-Host "`n⚠ Cannot update X402_SERVER_URL - Express URL not found" -ForegroundColor Yellow
        } elseif (-not $nextjsService) {
            Write-Host "`n⚠ Next.js service not found" -ForegroundColor Yellow
        }
    } else {
...
Write-Host "`nNote: Some settings may need to be set manually in Railway dashboard:" -ForegroundColor Yellow
Write-Host "  - Express Service → Settings → Root Directory: x402-server" -ForegroundColor White
